...
function Result(shape) {
  if (shape.toLowerCase() == "rectangle") return Rectangle;
  if (shape.toLowerCase() == "square") return Square;
  if (shape.toLowerCase() == "circle") return Circle;
  if (shape.toLowerCase() == "ellipse") return Ellipse;
  return () => {};
}
...
function Render(ele, fn) {
  return function (...args) {
    ele.innerHTML = fn(...args);
    return ele;
  };
}
...
const RenderQuestion =
  (...args) =>
  (step = 0) =>
    QUESTIONS[step]["description"](...args);
...
function* Next(i, len) {
  while (i < len) {
    ...
    Render(
      question,
      RenderQuestion(global_state.shape, {
...
actionBtn.addEventListener("click", () => {
  ...
  if (isDone.value == 2) {
    const ans = Result(global_state.shape)(
      global_state.width,
      global_state.height
    );
